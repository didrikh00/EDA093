...
/* Globals*/
static direction_t bus_dir; // Bus direction
static int tasks_on_bus; // Nmbr of tasks on bus
static struct lock bus_lock; // Lock for the bus
static struct condition wait_dir[NUM_OF_DIRECTIONS][NUM_OF_PRIORITIES] = {0}; // Conditions for directions and priority pairs
static int wait_count[NUM_OF_DIRECTIONS][NUM_OF_PRIORITIES] = {0}; // Count array for conditions 
...
void get_slot (const task_t *task) {
  ...
  while((tasks_on_bus == BUS_CAPACITY) || (tasks_on_bus > 0 && bus_dir == other_direction(task->direction))
  | ((task->priority == NORMAL) && (wait_count[other_direction(task->direction)][PRIORITY]>0))){
    // Block acces if:
    // - Buss full
    // - Tasks in other direction
    // - Task in other direction has Priority
    wait_count[task->direction][task->priority]++; // Inc. waiting tasks in dir, prio
    cond_wait(&wait_dir[task->direction][task->priority], &bus_lock); // Wait on condition
    wait_count[task->direction][task->priority]--; // Dec. waiting tasks in dir, prio
  }
  tasks_on_bus++; // Add task on bus
  bus_dir = task->direction; // Change the bus direction to that of the task
...
void release_slot (const task_t *task) {
  ...
  if(wait_count[task->direction][PRIORITY] > 0 ){
    // If there is a task waiting in direction with priority
    cond_signal(&wait_dir[task->direction][PRIORITY],&bus_lock);
  } else if((wait_count[other_direction(task->direction)][PRIORITY] > 0 ) && (tasks_on_bus == 0)){
    // If there is a task waiting in the other direction with priority
    cond_signal(&wait_dir[other_direction(task->direction)][PRIORITY],&bus_lock);
  } else if(wait_count[task->direction][NORMAL] > 0 ){
    // If there is a task in current direction withou priority
    cond_signal(&wait_dir[task->direction][NORMAL], &bus_lock);
  } else if((wait_count[other_direction(task->direction)][NORMAL] > 0 ) && (tasks_on_bus == 0)){
    // If there is a task waiting in the other direction withou priority
    cond_signal(&wait_dir[other_direction(task->direction)][NORMAL], &bus_lock);
  }
